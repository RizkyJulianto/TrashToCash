    ...
    <section class=" dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-5xl  justify-between lg:py-16 flex">
            <div class="left-col basis-[55%]">
                ...
                <form action="{{ route('add.wallet-redeem') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        <div class="sm:col-span-2">
                            <label for="type_wallet"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                                E-Wallet</label>
                            <select id="type_wallet" name="type_wallet"
                                ...
                                @foreach ($ewalletList as $data)
                                    <option value="{{ $data }}">{{ $data }}</option>
                                @endforeach
                            </select>

                            <x-input-error :messages="$errors->get('type_wallet')" class="mt-2" />
                        </div>

                        <div class="w-full">
                            <label for="phone_number"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nomer Telepon
                            </label>
